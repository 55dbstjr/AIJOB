...
0. **Plane 상태 먼저 조회** — 오늘 진행한 티켓 + 하위 티켓 상태를 Plane에서 확인 (Plane이 원본이므로 파일/TODO보다 먼저 싱크)
...
- **퇴근 시 0→7→8→9→10 순서 엄수** — 건너뛰거나 커밋부터 하지 말 것
...
- **티켓 관련 파일 생성/재생성 전 Plane 상태 먼저 확인** — work 지시서(`.claude/work/{프로젝트}/IW-XXX.md`) 등 티켓과 연동된 파일이 없을 때 "유실됐다"고 추측해 재생성하지 말 것. 없는 이유는 대부분 "작업 완료 후 삭제"(운영 규칙). 반드시 Plane 상태 조회 → 완료면 재생성 금지 + TODO 체크, 미완료면 사용자에게 질문
